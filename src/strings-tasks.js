...
function repeatString(str, times) {
  if (str.length === 0 || times < 0) return '';
...
function removeFirstOccurrences(str, value) {
  if (str.indexOf(value) !== -1) return str.replace(value, '');
  return str;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < array.length; i += 1) {
    ...
    } else newArray.push(array[i].toLowerCase());
  }
  return newArray.join('');
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((elem) => {
      ...
      if (elem >= 'A' && elem <= 'Z') {
        code =
          'A'.charCodeAt() + ((elem.charCodeAt() - 'A'.charCodeAt() + 13) % 26);
      } else if (elem >= 'a' && elem <= 'z') {
        code =
          'a'.charCodeAt() + ((elem.charCodeAt() - 'a'.charCodeAt() + 13) % 26);
      } else {
        code = elem.charCodeAt();
      }
      return code;
    })
